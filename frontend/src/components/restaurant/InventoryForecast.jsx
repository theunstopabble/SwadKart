...
const InventoryForecast = () => {
  ...
  const fetchForecast = async () => {
    try {
      setLoading(true);
      const res = await fetch(`${BASEURL}/api/v1/inventory-forecast/forecast`, { credentials: "include" });
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div className="flex-1">
            ...
            <p className="text-xs text-gray-400">Stock level predictions and reorder recommendations</p>
          </div>
          <button onClick={fetchForecast} disabled={loading} className="bg-primary hover:bg-primary/80 text-white px-4 py-1 rounded-lg text-xs font-bold">
              ...
              {recommendations.map((rec, i) => (
                <div key={i} className="flex justify-between items-center bg-gray-800 rounded-lg px-4 py-2">
                  ...
                  <div className="flex items-center gap-3">
                    <span className="text-xs text-gray-400">Current: {rec.currentStock}</span>
                    <span className="text-xs font-bold text-red-400">+{rec.recommendedQty} units</span>
                  </div>
                </div>
              ))}
            </div>
